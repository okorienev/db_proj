...
from app.models import get_session, PersonalCard, Field, FieldType


class ViewExtendedPd(AbstractAction):
    ACTION_NAME: str = "view-extended-pd"
    COMMAND_NAME: str = "view-extended-pd"
        ...
        while True:
            ...
            if name.isalpha() and surname.isalpha() and patronymic.isalpha():
                ...
                query = session.query(PersonalCard, Field, FieldType).filter(PersonalCard.card_id == Field.card_id,
                                                                             ...
                                                                             PersonalCard.is_archive == False,
                                                                             PersonalCard.surname == surname,
                                                                             FieldType.sensitive == False).all()
                if len(query) == 0:
